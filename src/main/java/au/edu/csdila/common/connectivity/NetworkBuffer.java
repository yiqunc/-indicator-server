package au.edu.csdila.common.connectivity;
...
public final class NetworkBuffer {
	...
	public static Map findServiceArea(SimpleFeatureSource network,
			...
			Double bufferDistance) throws IOException, NoSuchAuthorityCodeException, FactoryException, MismatchedDimensionException, TransformException, SchemaException {
		...
		Point pointOfInterest4326 = (Point) pointFeature.getDefaultGeometry();
		//MOD BY BENNY START
		
		//transfrom 4326 to 23855 before applying buffer around the feature point
	    ...
	    Point pointOfInterest = (Point)JTS.transform(pointOfInterest4326, transform28355);
		Geometry pointBuffer28355 = pointOfInterest.buffer(networkDistance + bufferDistance);
		//then change buffer back to 4326
		Geometry pointBuffer = JTS.transform(pointBuffer28355, transform4326);
		
		//query network within pointbuffer via 4326
		SimpleFeatureCollection networkRegion4326 = featuresInRegion(network, pointBuffer);
		//convert network into 28355 for future use
		SimpleFeatureCollection networkRegion = new ForceCoordinateSystemFeatureResults(networkRegion4326, CRS4326, false);
		networkRegion = new ReprojectingFeatureCollection(networkRegion, CRS28355);
		
		LOGGER.info("========  1 network seg for point {} number in buffer: {}", pointFeature.getID(), networkRegion.size());
