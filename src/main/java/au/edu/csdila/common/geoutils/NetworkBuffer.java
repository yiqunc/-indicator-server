...
public final class NetworkBuffer {
	 ...
	 */
	public static Map findServiceArea(SimpleFeatureCollection network,
		...
		SimpleFeatureCollection networkRegion;
		
		try {
			networkRegion = GeometryUtils.intersection(network, pointBuffer, -1);
		} catch (Exception e) {
		...
		if(networkRegion.size() == 0) return null;
		LOGGER.info("========  2 network seg for point {} number in buffer: {}", pointFeature.getID(), networkRegion.size());
